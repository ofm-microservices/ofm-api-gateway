...
// TracingModule wires OpenTelemetry tracing for api-gateway.
var TracingModule = fx.Options(
	fx.Provide(ProvideTracingConfig),
	fx.Invoke(InvokeInstallTracing),
)

// ProvideTracingConfig adapts api-gateway config to the shared tracing config.
func ProvideTracingConfig(cfg *config.Config) *sharedtracing.Config {
	tcfg := sharedtracing.DefaultConfig("api-gateway", cfg.App.Env)
...
// InvokeInstallTracing installs the shared tracer provider and propagator.
